...
export default function SolarDetectionPage() {
  ...
  const handleDetect = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        // Mock results
        const mockResults = {
          hasSolar: Math.random() > 0.3,
          confidence: (0.7 + Math.random() * 0.25).toFixed(2),
          panelCount: Math.floor(Math.random() * 20) + 1,
          area: (10 + Math.random() * 30).toFixed(1),
          capacity: (1.5 + Math.random() * 5).toFixed(1),
          co2Offset: (0.8 + Math.random() * 1.2).toFixed(1)
        }
  ...
  const handleExport = (format: string) => {
    ...
    alert(`Exporting results as ${format}...`)
  }
